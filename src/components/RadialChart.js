...
export default function RadialChart(props) {
  ...
  useEffect(() => {
    ...
    const currentRadialChart = d3
        .select(radialChart.current)
        .attr('width', 260)
        .attr('height', 260)
        .attr('viewBox', '0 0 260 260'),
      currentRadialChartHeight = parseInt(
        currentRadialChart.attr('height')
      ),
      currentRadialChartWidth = parseInt(
        currentRadialChart.attr('width')
      ),
    ...
    currentRadialChart
      ...
      .attr(
        ...
        d3
          ...
          .outerRadius(90)
          .startAngle(0)
          .endAngle((-(data * 360) * Math.PI) / 180)
      )
      .attr(
        'transform',
        `translate(${chartMidWidth} ${chartMidHeight})`
      )
      .attr('fill', '#FF0000');
    ...
    currentRadialChart
      .append('circle')
      .attr('cx', chartMidWidth)
      .attr('cy', chartMidHeight)
      .attr('r', 80)
      .attr('fill', 'white');
