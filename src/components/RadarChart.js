...
export default function RadarChart(props) {
  ...
  useEffect(() => {
    ...
    const currentRadarChart = d3
      ...
      currentRadarChartHeight = parseInt(
        currentRadarChart.attr('height')
      ),
      currentRadarChartWidth = parseInt(
        currentRadarChart.attr('width')
      ),
    ...
    for (let i = 0; i < 5; i++) {
      hexa
        ...
        .attr(
          ...
          hexaPoly(
            Array(6).fill(
              chartMidHeight - 40 - 22 * i - (i === 4 ? 12 : 0)
            )
          )
        )
        .attr(
      ...
      .attr(
        ...
        hexaPoly([
          ...data.map((item) =>
            item.value < 1
              ? 0.5
              : (item.value / valueMax) * (chartMidHeight - 40)
          ),
        ])
      )
      .attr(
