...
export default function LineChart(props) {
  ...
  const isSessionLengthLinear = data.every(
      (item) => item.sessionLength === data[0].sessionLength
    ),
    sessionMax =
      Math.max(...data.map((item) => item.sessionLength)) !== 0
        ? Math.max(...data.map((item) => item.sessionLength))
        : 1;

  useEffect(() => {
    lineChart.current.innerHTML = '';

    const currentLineChart = d3
        .select(lineChart.current)
        .attr('width', 260)
        .attr('height', 260)
        .attr('viewBox', '0 0 260 260'),
      currentLineChartHeight = parseInt(
        currentLineChart.attr('height')
      ),
      currentLineChartWidth = parseInt(
        currentLineChart.attr('width')
      ),
    ...
    const Curve = d3
      ...
      .y((d, i) =>
        isSessionLengthLinear
          ? currentLineChartHeight - 120 + i * 0.01
          : (d.sessionLength / sessionMax) * 125 * -1 +
            currentLineChartHeight -
            60
      )
      .curve(d3.curveNatural);

    // Gradient for custom stroke
    const linGrad = currentLineChart
      .append('defs')
    ...
    linGrad
      .append('stop')
      .attr('offset', '0%')
      .attr('stop-color', 'rgb(255 255 255 / 0.5)');
    linGrad
      .append('stop')
      .attr('offset', '100%')
    ...
    const backgroundLine = currentLineChart
      .append('rect')
    ...
    const focus = currentLineChart
      .append('circle')
      .attr('class', 'focus')
      .attr('r', 4)
      .attr('fill', 'white')
      .style('display', 'none');
    const focusCircle = currentLineChart
      .append('circle')
      .attr('r', 9)
    ...
    currentLineChart
      ...
      .on('mousemove', function (e) {
        const posX = d3.pointer(e)[0],
          arrValPosX = Math.round(
            (posX / currentLineChartWidth) * (data.length - 1)
          ),
          nearPosX =
            (currentLineChartWidth / (data.length - 1)) * arrValPosX,
          lineChartWindowPos =
            lineChart.current.getBoundingClientRect();
        ...
        focus
          ...
          .attr(
            'cy',
            isSessionLengthLinear
              ? currentLineChartHeight - 120
              : (data[arrValPosX].sessionLength / sessionMax) *
                  125 *
                  -1 +
                  currentLineChartHeight -
                  60
          ...
          .style(
            ...
            Math.round(
              parseInt(window.scrollX) +
                parseInt(lineChartWindowPos.left) +
                (parseInt(focus.attr('cx')) + 10) *
                  (lineChartWindowPos.width / currentLineChartWidth)
            ) + 'px'
          )
          .style(
            'top',
            Math.round(
              parseInt(window.scrollY) +
                parseInt(lineChartWindowPos.top) +
                (parseInt(focus.attr('cy')) - 30) *
                  (lineChartWindowPos.height / currentLineChartHeight)
            ) + 'px'
          )
    ...
    currentLineChart
      ...
      .attr(
        'x',
        (d, i) =>
          (currentLineChartWidth / 7) * i + currentLineChartWidth / 14
      )
      .attr('y', currentLineChartHeight - 16);
  });
