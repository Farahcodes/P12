...
export default function BarChart(props) {
  ...
  useEffect(() => {
    ...
    const currentBarChart = d3
        .select(barChart.current)
        .attr('width', 835)
        .attr('height', 320)
        .attr('viewBox', '0 0 835 320'),
      currentBarChartGraph = currentBarChart
        .append('svg')
        .attr('width', 700)
        .attr('height', 145)
        .attr('x', 40)
        .attr('y', 110),
      currentBarChartGraphHeight = parseInt(
        currentBarChartGraph.attr('height')
      ),
      currentBarChartGraphWidth = parseInt(
        currentBarChartGraph.attr('width')
      ),
    ...
    for (let i = 0; i < 3; i++) {
      currentBarChartGraph
        .append('line')
        .classed('line', true)
        .attr('x1', 0)
        .attr('y1', (currentBarChartGraphHeight / 2) * i)
        .attr('x2', currentBarChartGraphWidth)
        .attr('y2', (currentBarChartGraphHeight / 2) * i)
        .attr('stroke', '#DEDEDE')
        .style('stroke-dasharray', i < 2 ? 2 : 0);
    }

    // Create bar groups
    const chart = currentBarChartGraph
      .classed('graph', true)
      .selectAll('.bar')
      .data(data)
      .enter()
    ...
    chart
      ...
      .attr(
        ...
        (d, i) =>
          i *
            (56 +
              (currentBarChartGraphWidth - 56 * data.length + 32) /
                (data.length - 1)) -
          16
      )
      .attr('y', 0)
      .attr('width', 56)
      .attr('height', currentBarChartGraphHeight)
    ...
    chart
      ...
      .attr(
        'y',
        (d, i) =>
          currentBarChartGraphHeight -
          ((d.kilogram - kilogramMin) / (kilogramMax - kilogramMin)) *
            currentBarChartGraphHeight
      )
      .attr('width', 8)
      .attr(
    ...
    chart
      ...
      .attr(
        ...
        (d, i) =>
          currentBarChartGraphHeight -
          ((d.calories - caloriesMin) /
            (caloriesMax + 50 - caloriesMin)) *
            currentBarChartGraphHeight
      )
      .attr('width', 8)
      .attr(
    ...
    chart
      .on('mouseover', function (e, d, i) {
        ...
        d3.select(this.querySelector('.background'))
          .transition()
          .duration(50)
          .attr('opacity', 1);

        // Create tooltip with text
        const tooltipBarText = d3
          .select(currentTooltipBar)
          .append('div')
          .classed('tooltipBarText', true);
        tooltipBarText.append('span').text(d.kilogram + 'kg');
        tooltipBarText.append('span').text(d.calories + 'Kcal');
      })
      .on('mousemove', function (e) {
        // Translate tooltip
        d3.select('.tooltipBarText')
    ...
    currentBarChart
      ...
      .selectAll('.weight')
      .data([kilogramMax, kilogramAve, kilogramMin])
      .enter()
      .append('text')
      .classed('weight', true)
      .text((d, i) => d)
    ...
    currentBarChart
      ...
      .append('text')
      .classed('date', true)
      .text((d, i) => parseInt(d.day.split('-').slice(-1)))
      .attr(
        'x',
        (d, i) =>
          40 +
          i *
            (56 +
              (currentBarChartGraphWidth - 56 * data.length + 32) /
                (data.length - 1)) +
          (parseInt(d.day.split('-').slice(-1)) < 10 ? 8 : 4)
      )
